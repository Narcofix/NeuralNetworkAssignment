...
inFiles *apri_dir() {
  ...
  if ((hFind1 = FindFirstFile(Buffer2, &FindFileData)) ==
...
Retestr analizza_net(char *buff) {
  ...
  while ((ptr) != ptr2) {
    ...
    if ((ptr) == ptr2) break;
    struttura->dim =
        nedrealloc(struttura->dim, struttura->ndim++ * sizeof(int));
    struttura->dim[struttura->ndim - 1] =
        strtol(ptr, &ptr4, 10);  // salvo l' intero trovato
    ptr = ptr4;
  }
  ...
  struttura->pesi =
      nedmalloc((struttura->ndim - 1) * sizeof(tipopesoSinapsi *));
  for (lay = 0; lay < (struttura->ndim - 1); lay++) {
    struttura->pesi[lay] =
        nedmalloc(((struttura->dim[lay] + 1) * struttura->dim[lay + 1]) *
  ...
  while (*ptr != '>') {
    while (*ptr != '\n') {
      ...
      if (isdigit(*ptr)) {
        if (*(ptr - 1) == '-' || *(ptr - 1) == '+') ptr--;
        struttura->vals[lay1] =
            nedrealloc(struttura->vals[lay1], (lay + 1) * sizeof(tipoNeurone));
    ...
    if (lay != 0) {
      ...
      ++lay1;
      // struttura->vals = nedrealloc(struttura->vals, (lay1+1) *
      // sizeof(tipoNeurone* ));
      lay = 0;
    }
    ++ptr;
...
Retetrg splitta_test_set(Retetrg orig) {
  ...
  for (i = 0; i < perc; i++) {
    struttura->vals[i] = nedmalloc((orig->dim[0] + orig->dim_out) *
