...
tipopesoSinapsi casuale,max=(tipopesoSinapsi)0.1,min=(tipopesoSinapsi)-0.1;
unsigned int number;
rand_s(&number);
casuale = min + (  (max-min) * ( (tipopesoSinapsi)number / ((tipopesoSinapsi)(UINT_MAX) + (tipopesoSinapsi)1.0) ) );
return casuale;
}
...
tipoNeurone sigm_uni(tipoNeurone x){
	 return (tipoNeurone)1.0 / ((tipoNeurone)1.0 + (tipoNeurone)exp((tipoNeurone)-2.0 * x));
}
