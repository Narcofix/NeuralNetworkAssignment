...
/*******************//***************************************************************/ 
int main(){
 int dim_out=0,dim_out1=0,old_dim_out=0,k,epoca=0,sel_ts,sel_net,*occ,modo,modo1;
	char* nameF,*nameF1;
...
/*cerco i file da aprire*/
 ...
 if(sel_net!=-1 && sel_net!=-2)struttura=analizza_net(apri_il_net(INNF,sel_net));/*caso architettura da file*/
 sel_ts=apri_files_ts(INNF);/*lista file .ts disponibili*/
 struttura2=analizza_ts(apri_il_ts(INNF,sel_ts));/*bufferizzazione file ts*/
 if(struttura2->dim_out!=0 && (sel_net!=-1)){/*Scelta tipo di esecuzione: */
 ...
 if(sel_net==-1)R=CreaArchitettura_default(struttura2);
	else
	if(sel_net==-2)R=Crea_Architettura_CERCHI();
 /*capisco se usare i pesi di default o letti da file*/
	else 
		if(struttura->pesi==NULL) R=CreaArchitettura_file(struttura,1);
		else R=CreaArchitettura_file(struttura,0);
	/**************************************************/
APP:	
/*apprendimento*/      //a questo punto l' architettura é pronta per essere addestrata
...
/*inizializzo valori default irprop*/
  ...
  R->Fapp_pos=(tipoFattori)1.2;
	...
	modo1=NO_SPLIT;
	if(modo1!=NO_SPLIT)struttura3=splitta_test_set(struttura2); //split del training set (optionale)
  ...
  occ=estrai_patt_rand(struttura2->npatt);
   ...
   mse+=err_quad(R->ptr_str[R->nstrati-1],struttura2->vals[occ[k]],struttura2->dim[0]);
  }
  ...
  if(modo==IRPROP)aggiorna_pesi_rprop(R);
		...
		else{/*eseguo funzionamento sulla parte splittata per verificare errore su pattern sconosciuti*/
